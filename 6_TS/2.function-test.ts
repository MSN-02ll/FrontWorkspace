...
function racoonInfo(
    ...
    if(neutralize !== undefined){
    return(`이름 : ${name} , 무게 : ${weight} , 성별 : ${sex}, 중성화 : ${neutralize}`)
    } else{
    return(`이름 : ${name} , 무게 : ${weight} , 성별 : ${sex}`)
    }
}
...
//multiplyAll의 매개변수는 1개 의상의 매개인자가 
// 들어올 수 있습니다.
...
// fn함수는배열의 첫 번째를 제외한 나머지 값들에는
// 첫 번째 값을 더한 새로운 배열을 반환합니다.
