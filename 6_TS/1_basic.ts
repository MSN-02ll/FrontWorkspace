 ...
 * 
 *  1.타입주석
 *   - 타입스크립트에서는 변소 선언시 "타입"을 명시할 수 있다.
...
// 객체 타입주석
// {key:type}
let animal:{name:string}; // key 값은 문자열과 정수 밖에 안됨, 거의 문자열로 봐야함
animal = {name:'coco'};

/**
 *  2. 타입추론
 *   - 변수에 대입하는 값을 바탕으로 타입을 자동으로 추론하는 기능
 *   - 간단한 변수 선언시에는 타입주석을 작성하지 않고 타입 추론을
 *    활용하는 것이 좋다.
...
numTuple = [1,2,3];
// console.log(numTuple[3]); // 잘못된 인덱스 접근시 컴파일에러, 실행은 된다.
 ...
 *   전 단계에서 작동하므로 실행환경에서 동작하는 메서드에 대한
 *   검사는 수행하지 않는다.
 * 
 */
numTuple.push(1111);
...
// 객체의 속성에 수정불가옵션 추가
let readonlyObj:{name :string};
readonlyObj = {name:'kh'};
readonlyObj.name = "pyw";
...
//  - 단순히 값을 받기 위한 용도로만 사용하며, unknown에 할당된
