...
test2.onclick = function(){
    ...
    //js에서는 객체(or class) 안에 선언한 함수를 매서드라고 부른다.

    const pet = {
        petName : '코코',
        // petName is not defined
        eat : function(food){
            console.log(petName +"가 "+food+"맛있게 먹는다");
            // 메서드에서 this는 메서드를 호출한 객체의 주소값이 바인딩
        }
    };
    console.log(pet);
    pet.eat('츄르');
...
//객체는 생서된 이후, 자유롭게 속성을 "추가" 하거나, 삭제할 수있다.
...
test7.onclick = function(){
    ...
    생성자 함수
        ...
        new 연산자와 함꼐 호출해야한다.
        - 생성자 함수는 첫글자를 클래스와 같이 대문자로 작성하는 것이 관례
...
test8.onclick = function(){
    /* 
        javascript의 class 생성문법(es6 이상 버전)
    */
    class Animal{
        //생성자 함수(필수)
        constructor(kind){
    ...
    class Pet extends Animal{
        ...
        // 메서드부
        // getter/setter
        // #private의 경우 getter/setter를 다시 설정.
        setHobby(value){
            this.hobby = value;
        }
