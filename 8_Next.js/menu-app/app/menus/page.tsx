...
export default async function MnuListPage(){
    /**
     *  #1 메뉴리스트
     *   - Next.js에서 화면작업을 할때는,
     *   데이터베이스에서 조회가 필요한 부분과,
     *   필요 없는 부분을 나눠서 각각
     *   서버컴포넌트, 클라이언트 컴포넌트로 구분해줘야 한다.
     *   - page.tsx는 서버컴포넌트로 관례상 항상 추가해야하며,
     *   데이터베이스 조회작업 및 공용 레이아웃(layout.tsx)을 작성한다.
     */
     ...
     *   하는 기능을 추가했다.
     *   - 따라서 같은 api를 여러 컴포넌트에서 호출해도 실제 네트워크 요청은
     *   한번만 발생하여 기능을 최적화 할 수 있다.
     *   - fetch함수에 cache속성을 추가하지 않는 경우 cache:auto 속성이 적용된다.
     ...
     *   -{next : {revalidate:60}} => 60초동안 유지되는 캐시
     *  
     */
    //const initialMenus:Menu[] = await (await fetch(`http://localhost:8081/api/menus`,{cache:'force-cache'})).json();
