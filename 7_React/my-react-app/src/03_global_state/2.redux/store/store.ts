/**
 *  #1 Redux
 *   - 리액트를 통한 복잡한 적역상태 관리시 자주 사용되는 라이브러리
 ...
 *  - 액션은 sotre의 상태값을 변경하는 요청내용을 담은 객체
 ...
 *  - RTK(Redux Tool Kit)에서는 Reducer와 Action,
 *   Action Creater를 하나의 파일에서 정의.
 *  5) Dispatch(Action전송 함수)
 *  - Action을 Store에 전달하는 함수
 *  - Dispatch를 통해 Action이 전달되면 Action에서 정의한
 *   type의 reducer가 호출된다.
 *  6) Selector(구독)
