...
export default function ParentComponent(){
    ...
    // let message = "Hello from Parents";
     ...
     *   이 상태 값을 감지학 위한 용도로 선언한다.
     *   - 부모컴포넌트에서 여러 자식컴포넌트에 공유해야할 데이터가 있는
     *   경우 부모컴포넌트에서 state를 선언후 자식에게 props로 전달해야한다.
     * */
...
function ChildComponent({style, message, setMessage}:ChildProps){
    // console.log(props);
    // props. message = "마음에 안드네, .."; // props는 읽기전용(readOnly)으로 사용할 것

    const messageHandler = () => {
        message = "자식에서 수정"; // 단순한 변수값 변경은 리렌더링 대상이 아님
    }
        return (<div>
